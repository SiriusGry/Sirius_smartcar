...
bool stop_trafiic=false;
bool StopA =false ;  // 红绿灯
bool StopB =false ;  // 停车位
...
public:
  ...
  void stop_contr(const std_msgs::Int8::ConstPtr msg);

  geometry_msgs::Point get_odom_car2WayPtVec(const geometry_msgs::Pose &carPose);

private:
  ros::NodeHandle n_;
  ros::Subscriber odom_sub, path_sub, goal_sub, encoder_sub, final_goal_sub, line_sub, traffic_light_sub, key_sub;
  ...
  void lightCB(const std_msgs::String::ConstPtr& msg);
...
L1Controller::L1Controller()
{
  ...
  pn.param("goalRadius", goalRadius, 0.5);
  ...
  traffic_light_sub = n_.subscribe("/light", 1, &L1Controller::lightCB,this);
  key_sub = n_.subscribe("/teleop_key", 1, &L1Controller::stop_contr, this);
  ...
  goal_received = false;
  goal_reached = false;
  cmd_vel.linear.x = 1500;          // 0 for stop
...
void L1Controller::stop_contr(const std_msgs::Int8::ConstPtr msg){
  int key_flag = msg->data;
  if (key_flag == 1){
    StopA = true;
  }
  else if (key_flag == 2){
...
void L1Controller::controlLoopCB(const ros::TimerEvent &)
{
    ...
    if (goal_received)
    {
      ...
      if (foundForwardPt)
      {
        if (!goal_reached)
        {
          ...
          if (cmd_vel.angular.z > 145)
            cmd_vel.angular.z = 145;
          else if (cmd_vel.angular.z < 35)
            cmd_vel.angular.z = 35;
  ...
  else if (stop_flag == 10&&stop_trafiic== false){
    cmd_vel.linear.x = 1300;
    cmd_vel.angular.z = 89;
    pub_.publish(cmd_vel);
...
int main(int argc, char **argv)
{
  ...
  // Wait for the blocking thread to finish before exiting
